...
public class Smile1 extends Enemy {
    private int movementBuffer = 0;

    public Smile1 (int x, int y, GamePanel gp) {
        super(x, y, gp);
    }
    ...
    public void getImage() {
        try {
            image = ImageIO.read(getClass().getResourceAsStream("/res/enemy/slime1.png"));
            image2 = ImageIO.read(getClass().getResourceAsStream("/res/enemy/deadslime1.png"));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
        ...
        if (!dead) {
            switch (direction) {
                case "up":
                    ...
                    frame = image.getSubimage(80 * tick, 240, 80, 80);
                    break;
                case "down":
                    frame = image.getSubimage(80 * tick, 0, 80, 80);
                    break;
                case "left":
                    frame = image.getSubimage(80 * tick, 80, 80, 80);
                    break;
                case "right":
                    frame = image.getSubimage(80 * tick, 160, 80, 80);
                    break;
            }

        } else {
            frame = image2.getSubimage(80 * tick, 0, 80, 80);
        }
